...
export const UPDATES_LEDE =
  'Этот сервис не подключен к БГУ напрямую. Автоматизированный бот заходит на abit.bsu.by каждые 4–5 минут, снимает данные таблицы и сохраняет новый файл в репозиторий. Страница читает этот файл и показывает его на сайте.';
...
export const UPDATES_FOOT =
  'Сайт всегда старается показать самый актуальный снимок: сначала по прямой ссылке на коммит, потом через GitHub Pages, потом с ветки. Если один канал отстал — сайт использует другой. Если таблица на БГУ временно не отвечает — сайт отображает прошлые строки и помечает это в баннере.';
